...
2. From the HACS store, open **Rituals Perfume Genie** and install the latest version.
...
2. Search for “Rituals Perfume Genie” and select it.
...
## Troubleshooting
- **Integration missing from the add-in list**: confirm the files exist under `/config/custom_components/rituals_perfume_genie/`, reinstall from HACS if needed, and restart Home Assistant.
- **Authentication errors**: verify the Rituals credentials, or reauthenticate via **Settings → Devices & Services → Rituals Perfume Genie → Configure**.
...
## Notes
- The required `pyrituals` API client is vendored inside `custom_components/rituals_perfume_genie/pyrituals/`, so no external dependencies are downloaded at runtime.
